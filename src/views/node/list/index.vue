    ...
    <Card class="list" v-if="isReady">
        <Table :data="list" :columns="columns">
            <template slot-scope="{ row }" slot="totalTransfer">
                {{ row.totalTransfer | bytesToSize}}
            </template>
            <template slot-scope="{ row }" slot="usedTransfer">
                {{ row.usedTransfer | bytesToSize}}
            </template>
    ...
    export default {
        ...
        data () {
            return {
                ...
                columns: [
                    { type: 'index', width: 80 },
                    { title: '节点名称', key: 'name', minWidth: 80, maxWidth: 200 },
                    ...
                    { title: '节点总流量', slot: 'totalTransfer', width: 140 },
                    { title: '已用流量', slot: 'usedTransfer', width: 100 },
                    { title: '剩余流量', slot: 'residualFlow', width: 140 },
                    { title: '平均在线', key: 'averageOnline', width: 160 },
